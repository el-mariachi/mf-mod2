...
import { calcMoveCoords, calcSpeed, isCoordsEqual, nextCoordsByVector, roundCoords } from '@game/utils';
...
export default class Sprite implements Types.AnimatableOnCanvas
{
  ...
  animate (params: Types.SpriteAnimationParams | null)
  {
    ...
    const result = {params, reason: 'end'} as Types.SpriteAnimationProcessResult;
    if (params)
    {
      ...
      if (params.playMotion)
      {
        ...
        if (this._motions && typeof motion == 'string' && motion in motionTypes)
        {
          ...
          if (!(motion in this._motions))
          {
            ...
            if (motion in Types.IdleMotionType && Types.IdleMotionType.idle in this._motions)
            {
              motion = Types.IdleMotionType.idle;
            }
            else if (motion in Types.MoveMotionType && Types.MoveMotionType.move in this._motions)
            {
              motion = Types.MoveMotionType.move;
            }
            else if (motion in Types.AttackMotionType && Types.AttackMotionType.attack in this._motions)
            {
              motion = Types.AttackMotionType.attack;
            }
            else if (motion in Types.DeathMotionType && Types.DeathMotionType.death in this._motions)
            {
              motion = Types.DeathMotionType.death;
            }
            else if (motion in Types.DamageMotionType && Types.DamageMotionType.damage in this._motions)
            {
              motion = Types.DamageMotionType.damage;
            }
            else if (motion in Types.TurnMotionType && Types.TurnMotionType.turn in this._motions)
            {
      ...
      if (params.playMotion || (params.to && params.duration))
      {
        if (params.playMotion)
        {
          ...
          const {origin: motionOrigin} = params.playMotion.motion as Types.AnimationMotionParams;
        ...
        if (params.to)
        {
          params.to = (!Array.isArray(params.to) ? nextCoordsByVector(this._geometry.position, params.to) : params.to) as Types.Coords
          movementSpeed = calcSpeed(this._geometry.position, params.to, duration)
        }
        ...
        animation.process = new Promise<Types.SpriteAnimationProcessResult>(resolve =>
        {
  ...
  update (dt : number)
  {
    if (this._activeAnimation)
    {
      ...
      const isMotionCompleted = !hasMotion || this._activeAnimation.isMotionCompleted;
      ...
      if (hasMovement)
      {
        ...
        const nextPosition = roundCoords(calcMoveCoords(this._geometry.position, movementSpeed, dt));
  ...
  render ()
  {
      ...
      if (playMotion && !isMotionCompleted)
      {
        ...
        if (speed > 0)
        {
          ...
          if (once && frameInd >= framesCnt)
          {
            ...
            motionFrame = frames[frameInd % framesCnt];
        }
        else
          motionFrame = 0;
        ...
        (this._geometry.origin as Types.Geometry).position[framesAxisInd] = motionFrame * motionOrigin.size[framesAxisInd];
      }
    }
    ...
    if (origin)
    {
      this._ctx.drawImage(this._atlas, origin.position[0], origin.position[1], origin.size[0], origin.size[1], this.position[0], this.position[1], this.size[0], this.size[1]);
    }
    else
      this._ctx.drawImage(this._atlas, this.position[0], this.position[1], this.size[0], this.size[1]);
  }
}
