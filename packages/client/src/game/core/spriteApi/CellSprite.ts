...
import { cellCoords2PixelCoords, cells2pixels, mapCoords, nextCoordsByVector, relCoords } from '@game/utils'
...
export default class CellSprite extends Sprite {
  ...
  set cellGeometry (nextGeometry: Partial<Types.CellSpriteGeometry>)
  {
    ...
    {
      this._geometry.position = relCoords(mapCoords(), cellCoords2PixelCoords(nextGeometry.position))
    }
    ...
    if (params)
    {
      if (params.to)
      {
        if (!Array.isArray(params.to))
        {
          ...
          params.to = nextCoordsByVector(this._geometry.position, params.to) as Types.Coords
        }
        else
          params.to = relCoords(mapCoords(), cellCoords2PixelCoords(params.to)) as Types.Coords
      }
