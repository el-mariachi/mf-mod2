...
export default function spritesAnimationDemo(ctx: CanvasRenderingContext2D) {
  ...
  const coinView = createAnimatedView(ctx, Types.GameItemName.coin, [10, 9])
  ...
  makeStep(heroView.do(Behaviors.move2right))
    ...
    .then(() => makeStep(skeletonView.do(Behaviors.deathFromLeft, false)))
    .then(() => makeStep(heroView.do(Behaviors.move2right)))
    .then(() => makeStep(heroView.do(Behaviors.move2top)))
    .then(() => {
      setTimeout(() => coinView.toggle(false), 200)
      return makeStep(heroView.do(Behaviors.move2right))
    })
    .then(() => makeStep(heroView.do(Behaviors.look2bottom)))
