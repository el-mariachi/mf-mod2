  ...
  if (dirOrRot)
  {
    ...
    if (!isRotation)
    {
      ...
      if (Types.IdleMotionType.idle == type)
      {
        animatedBehavior = {
          playMotion: {
            motion: Types.IdleMotionType[`look2${direction}` as Types.IdleMotionType],
            once: false,
          },
        }
      }
      ...
      else if (Types.DamageMotionType.damage == type)
      {
        animatedBehavior = {
          playMotion: {
            motion: Types.DamageMotionType[`${type}From${dirCap}` as Types.DamageMotionType],
            once: true,
          },
        }
      }
      else if (Types.DeathMotionType.death == type)
      {
        animatedBehavior = {
          playMotion: {
            motion: Types.DeathMotionType[`${type}From${dirCap}` as Types.DeathMotionType],
            once: true,
          },
        }
      }
    ...
    else
    {
      ...
      if (Types.TurnMotionType.turn == type)
      {
        animatedBehavior = {
          playMotion: {
            motion: Types.TurnMotionType[`turn${dirCap}` as Types.TurnMotionType],
            once: true,
          },
        }
      }
